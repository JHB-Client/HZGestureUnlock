...
- (void)setArrow:(BOOL)arrow
{
    ...
    // 遍历子控件，改变其是否有箭头
    [self.subviews enumerateObjectsUsingBlock:^(HZCircle *circle, NSUInteger idx, BOOL *stop) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.circleSet enumerateObjectsUsingBlock:^(HZCircle *circle, NSUInteger idx, BOOL *stop) {
        
        [circle setState:CircleStateSelected];
        
        // 如果是登录或者验证原手势密码，就改为对应的状态
        if (self.type != HZCircleViewTypeSetting) {
...
{
    return [(HZCircle *)[self.circleSet firstObject] state];
}

#pragma mark - 清空所有子控件的方向
- (void)resetAllCirclesDirect
{
    [self.subviews enumerateObjectsUsingBlock:^(HZCircle *obj, NSUInteger idx, BOOL *stop) {
        [obj setAngle:0];
    }];
}
...
- (void)gestureEndByTypeSettingWithGesture:(NSString *)gesture length:(CGFloat)length
{
    if (length < CircleSetCountLeast) {
        ...
        if ([gestureOne length] < CircleSetCountLeast) { // 接收并存储第一个密码
            // 记录第一次密码
            [HZCircleViewConst saveGesture:gesture Key:gestureOneSaveKey];
            ...
            BOOL equal = [gesture isEqual:[HZCircleViewConst getGestureWithKey:gestureOneSaveKey]]; // 匹配两次手势
...
{
    NSString *password = [HZCircleViewConst getGestureWithKey:gestureFinalSaveKey];
...
- (NSString *)getGestureResultFromCircleSet:(NSMutableArray *)circleSet
{
    ...
    for (HZCircle *circle in circleSet) {
        // 遍历取tag拼字符串
...
- (void)connectCirclesInRect:(CGRect)rect lineColor:(UIColor *)color
{
    ...
    for (int index = 0; index < self.circleSet.count; index++) {
        ...
        // 取出选中按钮
        HZCircle *circle = self.circleSet[index];
...
-(void)calAngleAndconnectTheJumpedCircle{
    ...
    //取出最后一个对象
    HZCircle *lastOne = [self.circleSet lastObject];
    
    //倒数第二个
    HZCircle *lastTwo = [self.circleSet objectAtIndex:(self.circleSet.count -2)];
    ...
    HZCircle *centerCircle = [self enumCircleSetToFindWhichSubviewContainTheCenterPoint:center];
 ...
 *  @return 点所在的圆
 */
- (HZCircle *)enumCircleSetToFindWhichSubviewContainTheCenterPoint:(CGPoint)point
{
    HZCircle *centerCircle;
    for (HZCircle *circle in self.subviews) {
        if (CGRectContainsPoint(circle.frame, point)) {
            centerCircle = circle;
        }
    }
